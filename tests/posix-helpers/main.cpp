...
int main() {
    ...
    if (!tmpPrepared) {
        ...
        // ============================================================ 15
        section("15. 清理：测试自己建的临时目录要删掉（真机上 /userdata 是用户数据分区）");
        // 为什么单列一节：本测试在真机上允许把临时目录建在 /userdata 上（见第 8 节），
        // 若不自清理，每跑一次就永久留下一个装着 256KB 文件的新目录 —— 真机上实测过。
        ok(::unlink(target.c_str()) == 0, "清理：删掉测试文件");
        ok(::rmdir(dir.c_str()) == 0, "清理：删掉临时目录");
        ok(!exists(dir), "清理后目录确实不存在了");
    }
